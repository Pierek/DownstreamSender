...
def entity_wrapper(func):
	def wrapper(*args):
		...
		# subprocess.call([ConfluentKafkaConfig.BAT_LOCATION + "\\kafka-topics.bat", '--zookeeper',
		# 	KafkaConfig.ZOOKEEPER, '--delete', '--topic', args[0]], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

		# retry = 1
		# max_retries = 3		
		# describe_output = b'test'
		# while describe_output.decode() != '':
		# 	logger.info('(' + func.__name__ + ') Checking if topic ' + args[0] + ' was deleted ' +
		# 		'(retry nr ' + str(retry) + ')...')

		# 	describe_output = subprocess.check_output([ConfluentKafkaConfig.BAT_LOCATION + "\\kafka-topics.bat", '--zookeeper',
		# 		KafkaConfig.ZOOKEEPER, '--describe', '--topic', args[0]])
...
def get_data(query, server=DBConfig.SERVER, db=DBConfig.DB, user=DBConfig.USER, password=DBConfig.PASSWORD):
	if user is not None:
		conn = pyodbc.connect('DRIVER={{ODBC Driver 13 for SQL Server}};SERVER={0};DATABASE={1};UID={2};PWD={3}'.format(server, db, user, password))
	else:
		conn = pyodbc.connect('DRIVER={{ODBC Driver 13 for SQL Server}};SERVER={0};DATABASE={1};Trusted_Connection=yes'.format(server, db))
...
def send_feature(topic, kafka_key):
	...
	rowcount = get_scalar("""
	;WITH CTE2 AS (
	SELECT DISTINCT [AVS] = COALESCE(F.MIPSCode,F.GCATCode)
	FROM export.tblAPL_Feature F
	WHERE F.MIPSCode IS NOT NULL
		OR F.GCATCode IS NOT NULL
	)
	...
	SELECT
		F.FeatureId
		,[FeatureCode] = COALESCE(F.MIPSCode,F.GCATCode)
		,FT.FeatureTypeCode
...
def send_aftermarket_part(topic, kafka_key):
	...
	rowcount = get_scalar("""SELECT COUNT(*)
	...
	SELECT
		...
		,[APNPartServiceStatusCode] = PS1.PartServiceStatusCode
		,[APNPartServiceStatusDescription] = PS1.Description		
		,[DescriptionCode] = CONVERT(varchar(10), AP.TermId)
		,[ServiceComment] = LEFT(RD.Description,4000)
		,AP.LRServiceDecisionDate
		,AP.LRServiceDecisionUser
		...
		,[LRRequiresCataloguing] = ISNULL(AP.LRRequiresCataloguing,0)
		,[UpsuffixFlag] = CASE WHEN PL.EffectiveDateOut IS NOT NULL THEN CONVERT(bit,1) ELSE CONVERT(bit,0) END
		,AP.LRAccessoryFlag
	...
	LEFT JOIN export.tblPRT_PartLink PL
		INNER JOIN export.tblPRT_EngineeringPart EP
			...
			LEFT JOIN config.tblPartServiceStatus PS1
				ON PS1.PartServiceStatusId = EP.LRServiceStatusId
	...
	for run in runs:
		for _ in range(run):
			...
			if row['LRServiceDecisionDate'] is not None:
				final['msg']['srv_dcsn_dt'] = '{:%Y-%m-%d}'.format(row['LRServiceDecisionDate'])
			...
			final['msg']['srv_dcsn_usr'] = row['LRServiceDecisionUser']
			...
			if row['LRRequiresCataloguing'] is not None:
				final['msg']['req_cat_flg'] = row['LRRequiresCataloguing']
			elif AftermarketPartConfig.IS_DEFAULT_ACTIVE:
				final['msg']['req_cat_flg'] = AftermarketPartConfig.DEFAULT_BOOL
			else:
				final['msg']['req_cat_flg'] = None

			final['msg']['upsf_flg'] = row['UpsuffixFlag']
			...
			final['msg']['acc_flg'] = row['LRAccessoryFlag']
...
def send_engineering_part_usage(topic, kafka_key):
	...
	SELECT
		...
		,EPU.Quantity
		,[ActiveFlag] = CASE WHEN EPU.ActiveFlag = 1 THEN 'A' ELSE 'I' END
		,EPU.AddDate
		,EPU.FeatureSetId
		,[IsDerivedFromParent] = CASE WHEN EPU.UsageType = 'C' THEN 'T' WHEN EPU.UsageType = 'F' THEN 'F' END
...
def send_hierarchy(topic, kafka_key):
	...
	rowcount = get_scalar("SELECT COUNT(*) FROM export.tblCAT_Hierarchy WHERE AssemblyLevel BETWEEN 1 AND 4")

	hierarchy_query = """
	WITH CTE AS (
	SELECT DISTINCT
		[HierarchyPath] = CONVERT(varchar(100),NULL)
		,[HierarchyId] = 0
		,HierarchyCode = ''
		,[HierarchyTypeId] = -1
		,[TermId] = -1
	...
	SELECT
		[HierarchyPath] = CONCAT('0.',H2.ParentHierarchyId)
		,H2.HierarchyId
		,H2.HierarchyCode
		,H2.HierarchyTypeId
		,H2.TermId
	...
	SELECT
		[HierarchyPath] = CONCAT('0.',H2.ParentHierarchyId,'.',H3.ParentHierarchyId)
		,H3.HierarchyId
		,H3.HierarchyCode
		,H3.HierarchyTypeId
		,H3.TermId
	...
	SELECT
		[HierarchyPath] = CONCAT('0.',H2.ParentHierarchyId,'.',H3.ParentHierarchyId,'.',H4.ParentHierarchyId)
		,H4.HierarchyId
		,H4.HierarchyCode
		,H4.HierarchyTypeId
		,H4.TermId
	...
	SELECT
		...
		,C.HierarchyId
		,C.HierarchyCode
		,[HierarchyTypeId] = ISNULL(CONVERT(varchar(10),HT.HierarchyTypeId),1)
		,[HierarchyTypeDescription] = ISNULL(HT.Description,'Brand')
		...
		,C.AddDate
	FROM CTE C
	LEFT JOIN config.tblHierarchyType HT
...
@entity_wrapper
def send_section_part_intray(topic, kafka_key):
